...
trait Symbol extends MusicElem{
  val time = 0.0
//  override def play(midiPlayer:MidiPlayer) = {} // play nothing for Sympols
  override def play(instrument:Instrument=Piano) = {} // play nothing for Sympols
}
...
case class Pause(override val time:Double = 0.25) extends Symbol{
  override def play(instrument:Instrument = Piano) = instrument.midiPlayer.play(key=1, duration = duration, volume=0 )
